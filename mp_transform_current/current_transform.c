...
uint16_t CSTransform(uint16_t current_ma)
{
    ...
    {
        accumulated += RANGE1_WIDTH_mA * RANGE1_TRANSFORMATION_FACTOR;      // Results in Maximum range 1 valeu in tmA
        current_ma -= RANGE1_WIDTH_mA;

        if(current_ma >= RANGE2_WIDTH_mA)
        {
            accumulated += RANGE2_WIDTH_mA * RANGE1_TRANSFORMATION_FACTOR;
...
uint16_t InverseCSTransform(uint16_t current_tma)
{
    ...
    {
        accumulated += RANGE1_WIDTH_tmA * RANGE1_INV_TRANSFORMATION_FACTOR;      // Results in Maximum range 1 valeu in mA
